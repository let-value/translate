...
test("ngettext handles context-aware message pairs", () => {
    ...
    assert.equal(t.npgettext("company", msg`${1} apple`, msg`${1} apples`, 1), "1 Apple устройство");
    assert.equal(t.npgettext("company", msg`${2} apple`, msg`${2} apples`, 2), "2 Apple устройства");
});
...
test("npgettext substitutes values from the chosen plural form", () => {
    ...
    assert.equal(t.npgettext("ctx", msg`${1} apple`, msg`${2} apples for ${"Bob"}`, 1), "1 apple");
    assert.equal(t.npgettext("ctx", msg`${1} apple`, msg`${2} apples for ${"Bob"}`, 2), "2 apples for Bob");
});

test("npgettext returns default forms when translation missing", () => {
    const t = new Translator("ru", translations);
    assert.equal(t.npgettext("company", msg`${1} pear`, msg`${1} pears`, 1), "1 pear");
    assert.equal(t.npgettext("company", msg`${2} pear`, msg`${2} pears`, 2), "2 pears");
});
