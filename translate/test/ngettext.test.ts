...
test("ngettext substitutes values from the chosen plural form", () => {
    const t = new Translator("en", translations);
    assert.equal(t.ngettext(msg`${"Bob"} ${1} carrot`, msg`${2} carrots for ${"Bob"}`, 1), "Bob 1 carrot");
    assert.equal(t.ngettext(msg`${"Bob"} ${1} carrot`, msg`${2} carrots for ${"Bob"}`, 2), "2 carrots for Bob");
});
